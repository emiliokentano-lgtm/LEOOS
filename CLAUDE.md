...
| 5, 6, 7 | Sound cues are a CATALOGUE: label, description, tone-as-notes and rate limit are table entries, so the player knows how to play a sequence of notes and nothing about what any cue means. Adding one is a line, not a branch. | `packages/contracts/src/sound.ts` |
| 45 | A cue fires from the SAME PLACE that already updated the screen, after the server said yes — never on the click. A confirmation tone for something that was then refused tells an operator their status changed when it did not. And the settings screen reports what audio can ACTUALLY do: "your browser has not allowed sound yet", rather than a switch that is on and silent. | [notifications §5](docs/architecture/12-notifications.md), `apps/web/lib/notifications/cue-player.ts` |
| 9, 44 | The panic CUE is unmutable in the same four places the panic category is — contracts, stripped on the way in, stripped again on the way OUT so an old backup cannot silence anybody, and a DB CHECK. Failing OPEN is the correct direction for this one decision, and a test tampers with the stored row to prove it. | `apps/api/src/modules/notifications/preference.service.ts` |
| 33, 41 | The gates COMPOSE rather than replace: a notification-sourced cue must pass the notification policy AND the cue policy, so adding cues could not quietly overturn "a task is deliberately silent". Asserted over the WHOLE catalogue. | `packages/contracts/test/sound.test.ts` |
| 21, 44, 50 | A fan-out is written in CHUNKS, because one INSERT has a 65 535-parameter ceiling and the largest fan-out in the product is a PANIC — so the notification that matters most would be the first to stop working, and it would stop by throwing rather than by delivering fewer. Regression-tested at six thousand recipients, verified by removing the chunking. | `apps/api/src/modules/notifications/notification.service.ts` |
